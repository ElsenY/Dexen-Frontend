...
import { Camera, Mail, Pencil, Save, LogOut, Phone, Hash, KeyRound, Calendar, Clock, Filter, ArrowRight } from 'lucide-react';
...
const Profile: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEditForm({ ...updatedProfile, current_password: '', new_password: '' });
      setIsEditing(false);
    } catch (error: any) {
             ...
             <div className="absolute -bottom-16 left-8">
                <div className="relative group">
                  <div className="w-32 h-32 rounded-3xl overflow-hidden border-4 border-[#0f172a] shadow-2xl bg-slate-800">
                    <img
                      src={displayImage ? `data:image/png;base64,${displayImage}` : "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=150&h=150"}
                      alt="Profile"
          ...
          <div className="pt-20 px-8 pb-8 md:px-12 md:pb-12">
            {!isEditing ? (
              <div className="mb-8">
                ...
                <p className="text-slate-400 font-medium">{profile.email || 'Initializing account...'}</p>
              </div>
            ) : (
            ...
            <form onSubmit={handleSave} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider ml-1">System ID</label>
                  <div className="relative group">
                    <Hash className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600" size={18} />
                    <input value={profile.id} readOnly className="glass-input pl-11 !text-slate-500 cursor-not-allowed border-none shadow-none" />
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider ml-1">Corporate Email</label>
                  <div className="relative group">
                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600" size={18} />
                    <input value={profile.email} readOnly className="glass-input pl-11 !text-slate-500 cursor-not-allowed border-none shadow-none" />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#6366f1] transition-colors" size={18} />
                    <input
                      name="phone_number"
                      type="tel"
              ...
              {isEditing && (
                <div className="pt-6 border-t border-white/5">
                  <h3 className="text-lg font-bold text-white mb-6">Security Update</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-xs font-semibold text-slate-400 ml-1 uppercase">Current Password</label>
                      <div className="relative group">
                        <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#6366f1]" size={18} />
                        <input
                    ...
                    <div className="space-y-2">
                      <label className="text-xs font-semibold text-slate-400 ml-1 uppercase">New Password</label>
                      <div className="relative group">
                        <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-[#6366f1]" size={18} />
                        <input
                          name="new_password"
              ...
              <div className="pt-6 border-t border-white/5 flex gap-4">
                {isEditing ? (
                  <>
                    <button type="button" onClick={handleCancel} className="flex-1 bg-white/5 text-slate-300 py-3.5 rounded-xl font-semibold hover:bg-white/10 transition-all border border-white/10">
                      Cancel
                    </button>
                    <button type="submit" className="flex-1 bg-[#6366f1] text-white py-3.5 rounded-xl font-semibold hover:bg-[#4f46e5] shadow-lg shadow-[#6366f1]/20 transition-all flex items-center justify-center gap-2">
                      <Save size={18} /> Save Changes
                    </button>
                  </>
                ) : (
                  <button type="button" onClick={handleLogout} className="w-full bg-red-500/10 text-red-400 py-3.5 rounded-xl font-semibold hover:bg-red-500/20 transition-all border border-red-500/20 flex items-center justify-center gap-2">
            ...
            <div className="glass rounded-[32px] p-8 md:p-10 animate-fade-up animation-delay-100">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-sm">Review your shift performance</p>
                        </div>
                    </div>
                ...
                {isAttendanceLoading ? (
                    <div className="text-center py-20 flex flex-col items-center gap-4">
                        ...
                        <p className="text-slate-500 font-medium tracking-wide">Retrieving shift data...</p>
                    </div>
                ) : attendances.length === 0 ? (
                    <div className="text-center py-20 bg-white/5 rounded-[24px] border border-dashed border-white/10">
                        <p className="text-slate-500">No shift records detected for this period.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 gap-4">
                        {attendances.map((att) => (
                            <div key={att.id} className="group flex items-center justify-between bg-white/5 border border-white/5 rounded-[24px] p-5 hover:bg-white/[0.08] hover:border-[#6366f1]/30 transition-all duration-300">
                                <div className="space-y-1">
                                    ...
                                    <div className="flex items-center gap-6 text-slate-400 text-sm italic">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span>In: <span className="text-emerald-400/90 font-mono not-italic">{formatTime(att.check_in)}</span></span>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <div className="w-2 h-2 rounded-full bg-rose-500"></div>
                                            <span>Out: <span className="text-rose-400/90 font-mono not-italic">{formatTime(att.check_out)}</span></span>
                                        </div>
                                    </div>
                                </div>
                                <div className="hidden sm:flex flex-col items-end opacity-0 group-hover:opacity-100 transition-opacity">
                                    <span className="text-[10px] uppercase font-bold text-[#6366f1] tracking-widest bg-[#6366f1]/10 px-3 py-1.5 rounded-lg border border-[#6366f1]/20">Record ID: {att.id.slice(0, 8)}</span>
                                </div>
                            </div>
                        ))}
                    </div>
